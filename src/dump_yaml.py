...
def load_external_ontologies(source=external_ontologies_dict, external_ontology_path=None):
    for name, external_ontology in source.items():
        ...
        for current_type in current_schema.schema.types.values():
            if 'uri' in current_type and (current_type['uri'] not in URIs_to_ontologies) and (
                    current_type['from_schema'] != current_from_path):
        ...
        for current_class in current_schema.schema.classes.values():
            if 'class_uri' in current_class and (current_class['class_uri'] not in URIs_to_ontologies) and (
                    current_class['from_schema'] != current_from_path):
        ...
        for current_slot in current_schema.schema.slots.values():
            if 'slot_uri' in current_slot and (current_slot['slot_uri'] not in URIs_to_ontologies) and (
                    current_slot['from_schema'] != current_from_path):
...
def get_graph(graph_to_read):
    ...
    for root, _, files in os.walk(folder_to_visit):
        for name in files:
            ...
            if any(current_file_path.endswith(suffix) for suffix in ['.ttl', '.nt', '.rdf', '.owl', '.n3', '_RDF']):
...
def add_str_to_single(obj_in, pred, string_to_store, source_mapping=None):
    ...
    if value_is_valid(string_to_store, current_datatype, pred, obj_in['name']):
        if current_slot not in obj_in or (
                current_slot in obj_in and isinstance(obj_in[current_slot], str) and obj_in[current_slot].endswith(
                "defined.")):
            obj_in[current_slot] = string_to_store.strip()
        else:
            obj_in.setdefault("comments", defaultdict(set))[current_slot].add(string_to_store.strip())
    else:
        obj_in.setdefault("comments", defaultdict(set))[current_slot].add(string_to_store.strip())
...
def convert_annotations(annotations):
    if not isinstance(annotations, dict):
        return annotations
    return {k: {'tag': k, 'value': convert_annotations(v)} for k, v in annotations.items()}
...
class GraphCharacterizer:
    def __init__(self, args):
        ...
        if args.okn_registry_id:
            ...
            self.schema['description'] = post.get('description', '')
            ...
            current_url = None
            if post.get('contact', {}).get('email'):
                current_url = 'mailto:' + post['contact']['email']
            elif post.get('contact', {}).get('github'):
                current_url = post['contact']['github']
            if current_url:
                self.schema.setdefault('contributors', []).append(current_url)
        else:
    ...
    def _build_indexes(self):
        ...
        for s, o in tqdm.tqdm(self.g.subject_objects(predicate=RDF.type), desc="Indexing types"):
    ...
    def add_str_to_multiple(self, obj_in, pred, string_to_store):
        ...
        if value_is_valid(string_to_store, current_datatype, pred, obj_in['name']):
            if current_slot == 'exact_mappings':
                ...
                if current_curie in {obj_in.get(k, '') for k in ['class_uri', 'slot_uri', 'uri']}:
                    return
    ...
    def process_ontologies(self):
        for entity in tqdm.tqdm(self.g.subjects(RDF.type, OWL.Ontology), desc='Ontologies'):
            for subj, pred, obj in self.g.triples((entity, None, None)):
    ...
    def add_slot(self, subj_key, subj_uri=None, subj_title=None, extra_info=None, from_schema=None):
        ...
        target['name'] = subj_key
        if subj_uri: target['slot_uri'] = str(subj_uri)
        if from_schema:
            target['from_schema'] = from_schema
            target['imported_from'] = from_schema
            target.pop('title', None)
    ...
    def process_classes(self):
        for class_type, extra_info in CLASS_TYPES.items():
            ...
            for entity in tqdm.tqdm(self.g.subjects(RDF.type, class_type), desc=f'Classes {str(class_type)}'):
                if entity in datatype_to_type: continue
                if isinstance(entity, BNode) or str(entity).startswith('_:'): continue

                # Check restriction manually since we are iterating subjects
                if (entity, RDF.type, OWL.Restriction) in self.g: continue

                subj_uri, subj_key = self.produce_curie_key(entity)
                if subj_uri in URIs_to_ontologies: continue

                if subj_key in self.schema['classes']:
                    cls = self.schema['classes'][subj_key]
                    if "title" in cls and "but has not itself been defined" not in cls['title']:
                ...
                for _, pred, obj in self.g.triples((entity, None, None)):
                    ...
                    if pred in SLOTS_TO_PREDICATES_SINGLE:
                        ...
                        if isinstance(obj, BNode) or str(obj).startswith('_:'): continue
                        if obj_key != subj_key:
                            if not check_for_cycles(subj_key, obj_key):
                                subclass_tree[obj_key].add(subj_key)
                            try:
                                self.check_for_import(str(obj))  # Using str(obj) as URI approx
                            except KeyError:
                                pass
                            self.schema['classes'][subj_key]['is_a'] = obj_key
                        if (obj_key not in self.schema['classes']) and (str(obj) not in URIs_to_ontologies):
                            self.add_class(obj_key, str(obj),
    ...
    def process_types(self):
        for class_type, extra_info in TYPE_TYPES.items():
            for entity in tqdm.tqdm(self.g.subjects(RDF.type, class_type), desc=f'Types {str(class_type)}'):
                if (entity, RDF.type, OWL.Restriction) in self.g: continue
                if isinstance(entity, BNode) or str(entity).startswith('_:'): continue
    ...
    def process_slots(self):
        for slot_type, extra_info in SLOT_TYPES.items():
            for entity in tqdm.tqdm(self.g.subjects(RDF.type, slot_type), desc=f'Predicates ({str(slot_type)})'):
                ...
                if entity in datatype_to_type: continue
                if subj_uri in URIs_to_ontologies: continue
                if str(entity).startswith(str(RDF) + '_'): continue
    ...
    def check_for_import(self, type_uri):
        ...
        target_entity = URIs_to_entities.get(type_uri, "")
    ...
    def add_to_range(self, pred_curie_key, obj_curie_key):
        ...
        if pred_uri in URIs_to_ontologies:
            return

        if obj_curie_key not in self.schema['slots'][pred_key]['any_of']:
    ...
    def assemble_counts(self):
        ...
        for s, p, o in tqdm.tqdm(self.g, desc="Analyzing Triples"):
            ...
            if p in skip_predicates: continue
            p_str = str(p)
            if p_str.startswith(str(RDF) + '_'): continue
            ...
            if s_types_raw:
                for st in s_types_raw:
                    if st in CLASS_TYPES or st in SLOT_TYPES or st == OWL.NamedIndividual: continue

                    st_curie, st_key = produce_curie_key(st)

                    self.schema['annotations']['counts']['classes'][st_curie] += 1
                    if st_curie not in self.schema['annotations']['examples']['classes']:
                        self.schema['annotations']['examples']['classes'][st_curie] = str(s)
            ...
            if len(subject_types_filtered) > 1:
                self.multiple_typed_object_counts[frozenset(subject_types_filtered)] += 1
            ...
            if p_curie not in URIs_to_entities and p_key not in self.schema['slots']:
            ...
            if isinstance(o, (URIRef, BNode)):
                ...
                if o_types_raw:
                    for ot in o_types_raw:
                        if ot in CLASS_TYPES or ot in SLOT_TYPES: continue
                        ot_curie, ot_key = produce_curie_key(ot)
                        object_type_uris_keys.append((ot_curie, ot_key))
            else:
                object_datatype = get_object_datatype(o)
                object_type_mapping, current_import, _ = linkml_type_mapping(object_datatype)
                if current_import:
                    self.schema['imports'].add(current_import)
                ...
                if dt_key not in self.schema['types'] and dt_curie not in URIs_to_ontologies:
            ...
            if has_types:
                for st_curie, st_key in subject_type_uris_keys:
                    # Ensure class exists
                    if st_curie not in URIs_to_ontologies:
                    ...
                    if object_type_uris_keys:
                        for ot_curie_key in object_type_uris_keys:
                            self.account_for_triple((st_curie, st_key), pred_curie_key, ot_curie_key, example)
                    else:
                        self.account_for_triple((st_curie, st_key), pred_curie_key, (None, 'untyped'), example)

            else:
                if object_type_uris_keys:
                    for ot_curie_key in object_type_uris_keys:
                        self.account_for_triple(None, pred_curie_key, ot_curie_key, example)
                else:
                    self.account_for_triple(None, pred_curie_key, (None, 'untyped'), example)
    ...
    def convert_slot_dicts(self):
        for key, slot_dict in self.schema["slots"].items():
            ...
            slot_dict['any_of'] = [{'range': typename[1] if isinstance(typename, tuple) else typename}
            ...
            slot_dict['union_of'] = [typename for typename in list(slot_dict['union_of'])]
    ...
    def clean_up_json(self):
        ...
        if 'comments' in self.schema:
            self.schema['comments'] = [
                (k + ': ' + v) for k, vals in self.schema['comments'].items() for v in vals
            ]
